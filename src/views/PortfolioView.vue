<template>
  <div class="min-h-screen bg-gradient-to-br from-accent-50 via-secondary-50 to-primary-50">
    <!-- Header -->
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <nav class="hidden md:flex space-x-8">
            ...
            <router-link to="/#about" class="text-gray-700 hover:text-primary-600 transition-colors">About</router-link>
    ...
    <section class="py-20 relative overflow-hidden">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="text-center mb-16 animate-fade-in-up">
          ...
          <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-primary-600 via-secondary-600 to-accent-600 bg-clip-text text-transparent mb-6 font-poppins">
            Complete Portfolio
          </h1>
          <p class="text-lg text-gray-700 max-w-3xl mx-auto leading-relaxed font-light">
            Explore my entire collection of web development projects. Each project represents a unique challenge solved with modern technologies and best practices.
          </p>
          ...
          <div class="mt-8 inline-flex items-center space-x-2 bg-white/70 backdrop-blur-sm px-6 py-3 rounded-full shadow-md">
            ...
            <span class="text-gray-700 font-medium">{{ totalProjects }} Total Projects</span>
          </div>
        </div>

        <!-- Filter Options (Future Enhancement) -->
        <div class="mb-12 flex justify-center space-x-4 animate-fade-in-up animation-delay-200">
          <button 
          ...
          <button 
            @click="filterCategory = 'web-development'"
            :class="[
              'px-6 py-2 rounded-full font-medium transition-all duration-300',
          ...
          <button 
            @click="filterCategory = 'wordpress'"
            :class="[
              'px-6 py-2 rounded-full font-medium transition-all duration-300',
        ...
        <!-- Empty State -->
        <div v-if="!loading && !error && (!filteredProjects || filteredProjects.length === 0)" class="text-center py-12">
...
// Set document title
onMounted(() => {
  document.title = 'Portfolio - My Work | John Doe'
})
...
const defaultProjects = [
  {
    ...
    technologies: ['Vue.js', 'Node.js', 'MySQL', 'Stripe API']
  },
  {
    id: 'project-2',
    ...
    thumbnail: 'Kissel-nologo.png',
    image: 'Kissel-nologo.png',
    technologies: ['Vue 3', 'Firebase', 'Tailwind CSS']
  },
  {
    id: 'project-3',
    ...
    thumbnail: 'Kissel-nologo.png',
    image: 'Kissel-nologo.png',
    technologies: ['WordPress', 'PHP', 'Advanced CSS', 'JavaScript']
  },
  {
    id: 'project-4',
    ...
    thumbnail: 'Kissel-nologo.png',
    image: 'Kissel-nologo.png',
    technologies: ['Laravel', 'Vue.js', 'MySQL', 'SMS API']
  },
  {
    id: 'project-5',
  ...
  },
  {
    ...
    thumbnail: 'Kissel-nologo.png',
    image: 'Kissel-nologo.png',
    technologies: ['React', 'Chart.js', 'REST APIs', 'WebSockets']
  },
  {
    id: 'project-7',
    title: 'Blog Platform',
  ...
  },
  {
    ...
    description: 'Complete LMS solution for online courses with video streaming, quizzes, and progress tracking.',
    category: 'Web App',
    projectLink: 'https://example.com',
    thumbnail: 'Kissel-nologo.png',
    image: 'Kissel-nologo.png',
...
const filteredProjects = computed(() => {
  if (filterCategory.value === 'all') return allProjects.value
  
  return allProjects.value.filter(project => {
    const category = project.category?.toLowerCase() || ''
    if (filterCategory.value === 'wordpress') {
      return category.includes('wordpress')
    } else if (filterCategory.value === 'web-development') {
      return category.includes('web') && !category.includes('wordpress')
    }
    return true
  })
})
...
onMounted(async () => {
  try {
    ...
    // Silently fall back to default projects data
  }
})
</script>
